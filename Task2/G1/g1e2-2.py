...
def print_rdd(rdd):
    ...
    dataFrame = sqlContext.createDataFrame(rdd,  
                    "carrier:string, delay:float, total:int, avg_delay:float")
    dataFrame.show() 
    ...
    for (x, y, z, w) in airlines:
        print("%s, %f, %f,%d" % (x, y, w, z))
    print('==========XYZ E===================')
...
def updateFunction(newValues, runningCount):
    ...
    values, counter, avg_delay = runningCount 
...
filtered = lines.map(lambda line: line.split("\t"))\
        		.map(lambda word: (word[0]+" " + word[1], (float(word[7]), 1, 0.)) )\
        		.updateStateByKey(updateFunction)\
                .map(lambda (x, y): (x, float(y[0]), int(y[1]), float(y[2])) )
